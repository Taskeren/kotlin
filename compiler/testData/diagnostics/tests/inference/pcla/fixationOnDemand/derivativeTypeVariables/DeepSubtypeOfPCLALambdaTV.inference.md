...
1. Choose `TypeVariable(OT)` with `WITHOUT_PROPER_ARGUMENT_CONSTRAINT`
...
1. Choose `TypeVariable(PNT)` with `WITHOUT_PROPER_ARGUMENT_CONSTRAINT`
...
2. Choose `TypeVariable(OT)` with `READY_FOR_FIXATION_LOWER`
    1. `TypeVariable(PNT)` is `READY_FOR_FIXATION_UPPER`
...
4. Combine `TypeVariable(PNT) <: TypeVariable(OT)` with `TypeVariable(OT) == ScopeOwner`
    1. `TypeVariable(PNT) <: ScopeOwner`
5. Choose `TypeVariable(PNT)` with `READY_FOR_FIXATION_UPPER`
...
1. `kotlin/Unit <: kotlin/Unit` _from LambdaArgument_
2. Choose `TypeVariable(OT)` with `READY_FOR_FIXATION_LOWER`
    1. `TypeVariable(PNT)` is `READY_FOR_FIXATION_LOWER`
...
5. Choose `TypeVariable(PNT)` with `READY_FOR_FIXATION_LOWER`
...
5. Combine `TypeVariable(OT) == ScopeOwner` with `TypeVariable(PNT) <: CapturedType(in TypeVariable(OT))`
    1. `TypeVariable(PNT) <: CapturedType(in ScopeOwner)`
6. Choose `TypeVariable(PNT)` with `READY_FOR_FIXATION_UPPER`
...
10. Combine `TypeVariable(PNT) <: BaseType` with `BaseType <: TypeVariable(PNT)`
    1. `TypeVariable(PNT) == BaseType`
11. Choose `TypeVariable(PNT)` with `READY_FOR_FIXATION_LOWER`
...
1. Choose `TypeVariable(CT)` with `READY_FOR_FIXATION_LOWER`
